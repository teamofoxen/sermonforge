...
- **One regression caught live and fixed — the notebook drawer went deaf.** `useModalA11y` keyed its whole focus lifecycle on the `onClose` identity, and every workspace overlay passes an inline arrow (new identity per render). The drawer re-renders the workspace on every keystroke, so each character re-ran the dialog's close-then-open focus dance: focus was yanked from the textarea to the "Study notebook" tab and every later keystroke died on the button (reproduced in the real preview; a Space there would even switch notebooks). Fix: the hook binds its lifecycle to the dialog's mount (`{open && <Dialog/>}` — mount IS open/close) and keeps the latest `onClose` in a ref for Escape. Also heals the quieter form: Finish/Map focus-restore no longer degrades when the save chip or export state re-renders mid-open. Regression test added (`workspace-a11y.test.tsx`, fail-before/pass-after verified); live re-walk: 25 chars typed with zero focus loss, Escape closes, focus restores to the summon control (drawer AND map).
- **Seven stale pre-remediation doc claims corrected** (the Session-2/3 code moves obsoleted them): `docs/SYSTEMS/database.md` Cross-System Dependencies still taught the retired dev-throw/prod-warn-drop `buildUpdate` law from `electron/main.js`; `docs/REFERENCE/schema.md` (×2) and `docs/SYSTEMS/series-planner.md` (×2) still pointed `validateAndCommit` / `SERMON_SEARCH_COLUMNS` / `seriesSermonOrderBy` / `buildUpdate` at `electron/main.js` instead of `electron/persistence.cjs`; `CLAUDE.md`'s database row carried the same stale location + law; `transport/wrangler.toml` still called `ADMIN_TOKEN` a "query token" (query auth is rejected since Session 5 — it is a Bearer header token).
- Verdict: PASS WITH SMALL CORRECTIONS. Suite 515 → 516 green. Operator actions remain: rotate `INGEST_TOKEN` + `ADMIN_TOKEN`, deploy the updated Worker, push.

---

## 2026-07-13 — fix: improve workspace accessibility and trust (remediation session 6/6)

- **Part A — the blocking overlays carry the house dialog contract.** Sermon start, the Study → Anchor handoff, Sermon Finish, the Sermon Map, and the notebook drawer all adopt `useModalA11y`: focus enters the dialog predictably, Tab is trapped (the obscured writing surface is keyboard-unreachable), Escape dismisses, focus restores to the invoking control, and each carries `role="dialog" aria-modal="true"`. The handoff's nested ESV-key-modal Escape guard is preserved (one Escape never closes both layers); threshold screens still refuse window-level Enter (a stray keypress must not consume a first visit). The hook's empty-dialog fallback gained a real fix: focusing a bare container silently no-opped (no tabindex) — it now takes `tabindex="-1"` so focus genuinely leaves the background.
- **Part B — every visible label owns its control.** `htmlFor`/`id` associations added across: Setup's ESV API-key field (KeyInput now forwards `id`), New Sermon (Chapter:verse, Date), New Series (Series title, Year), and the planner — book/topical Outline fields (Theme, Big Idea, Overview, Series title) plus the REPEATED Section and Sermon row controls with row-scoped ids (`sec-<id>-title` …, `sermon-<id>-ref` …); the commentary-outline textarea gained an aria-label. Live-verified in the preview: 14/14 `label[for]` associations resolve on the Outline, zero orphans. (No new field primitive: exactly two row components needed ids — direct association over fresh abstraction.)
- **Part C — trust wording:** already honest since Session 1 (the boundary's "nothing is lost" was replaced with the saved-work/last-edits distinction; `error-boundary-trust.test.tsx` pins it) — verified intact, nothing further.
- **Part D — fixture fidelity.** `SermonWritingSurfaceFixture`'s local `deriveQuestionStates` reimplementation deleted; the map states now come from the production `deriveQuestionStatesFromSermon` through the existing sermon-shape adapter — which also kills the `[object Object]` canvas preview (the local copy had no indented-canvas dispatch and stringified the envelope). Fixture map render asserted clean in tests; the fixture keeps its honest "preview only" scope.
- **Cognitive-continuity limit honored:** no re-entry feature built; the four open questions (what a pastor needs after 48–72h away; interpretation changing under an existing Outline/Body; a "where this stands" summary; whether Map + Your-work + handoff already suffice) recorded in `docs/PROPOSALS/re-entry-open-questions.md` — code waits for real pastor evidence.
- **Verification:** 10 new jsdom tests (focus entry/Escape/restore per overlay + accessible names + fixture fidelity; suite 505 → 515 green) plus a REAL-preview keyboard walk at 1024×700: focus lands on "Begin →" inside the aria-modal start overlay; Escape dismisses; the map opens with focus inside, Tab from the last of 87 focusables wraps to the first (containment), Escape restores focus to the map-summon control; dark mode applied with no horizontal overflow on the writing surface or planner. Lint 0; spine-integrity OK; `npm start` boots clean. No visual redesign, no walk-order change, no renamed outcomes, no invalidation/badge/version-history/AI-summary systems.

---

## 2026-07-13 — build: harden release and network boundaries (remediation session 5/6)

- **Part A — reproducible install.** `package.json` gains `engines.node >= 20` (policy: CI pins the 20 LTS line; local dev on any newer major works; the shipped app runs Electron's own Node). Both workflows install with **`npm ci`** — CI now FAILS on package.json/lock drift instead of silently healing it. `npm ci` verified locally on this machine (plus the post-install `@electron/rebuild` step README documents). `build/icon.ico` + `build/entitlements.mac.plist` confirmed git-tracked; macOS `.icns` stays deterministically generated in-workflow from the tracked iconset. README setup section rewritten around `npm ci` + the Node policy (its `npm run dev` description was already accurate).
- **Part B — release gates.** `build.yml` gains a `gates` job on the exact tagged commit — `npm ci`, full suite, the production-SQLite + migration/recovery matrix (named step), lint, spine-integrity, Vite production build — and BOTH platform builds `needs: gates`. The Windows job packages with `--publish never`, runs the new **bounded packaged smoke** (`scripts/packaged-smoke.cjs` + an `SF_SMOKE=1` hook in main.js: the real unpacked app must launch, initialize/migrate schema, expose the preload bridge, render, and exit cleanly, in an isolated `SF_SMOKE_USERDATA`), and only then publishes. Smoke executed locally against the real build: `SF_SMOKE_RESULT ok=true schema=33 preload=true rendered=true`, exit 0 — which also gives the Tier-1 boot/recovery fixes (2026-07-05, previously reasoning-verified only) their packaged execution evidence. macOS signing/notarization remain CI-only (no local environment) — documented, not claimed.
- **Part C — telemetry boundary is executable.** `electron/telemetry/events.js` is now a schema registry (exact names, keys, types, length caps); `bus.emit()` validates BEFORE local persistence and the Worker validates the mirrored table BEFORE D1 (parity pinned by test). Unknown names/keys/types and over-cap strings reject; every string is short-capped except the documented crash line (≤500 chars), so sermon-shaped payloads structurally cannot fit. **Retired never-emitted vocabulary:** `panel-time`, `field-time`, `sermon-create`, `sermon-finish` (zero callers); flag/form shapes gain field allowlists + caps and the whole-rejection rule; `lastAiCall` no longer accepted (AI removed 2026-05-09; D1 column stays historical). privacy.md updated to the enforced two-event truth.
- **Part D — worker admin auth.** `/inbox` authenticates via `Authorization: Bearer` only; a `?token=` query is REJECTED outright (401 `token_in_query_rejected`) so the log-leaking shape dies; `transport/inbox.html` sends the header. No tokens in repo (test asserts); `.dev.vars` gitignored and untracked (test asserts). **Rotation of the previously exposed `INGEST_TOKEN`/`ADMIN_TOKEN` remains an out-of-repo operator task, still open.**
- **Part E — bounded ESV requests.** New `electron/crosswayFetch.cjs` — the one Crossway helper (AbortController timeout, caller-signal cancellation, injectable fetch); both call sites (passage-fetch, key verification) route through it. Every request ends as success / controlled error / timeout / cancellation; timeout maps to the existing "offline" plain-language voice (renderer copy unchanged).
- Tests +31 (worker boundary incl. header-auth + query-rejection + schema gates + sermon-shape exclusion; telemetry schema matrix; crossway endings; release-pipeline shape scans). Suite 474 → 505 green; lint 0; spine-integrity OK; `npm ci` + `npm test` + `npm run build` + packaged smoke + `npm start` all verified locally.

---

## 2026-07-13 — test: prove database migration and recovery (remediation session 4/6)

- **The migration/recovery matrix now EXECUTES production code against real SQLite files** (`tests/persistence/migration-recovery.test.ts`, 10 tests): the full ladder + its transaction wrapper (`persistence.cjs migrate()`), the boot recovery (new `electron/dbRecovery.cjs`), and the legacy resolver (`electron/dbMigration.js`). Rollback is observed on disk, not inferred from source; the old source-scan guards remain as cheap tripwires only.
- **Fixture matrix** (programmatic temp DBs, each stating its historical contract): pre-search v21 (v22 creates + backfills `sermon_search`); pre-soft-delete v23 (`deleted_at` arrives, rows live); pre-series-normalization v27 (limbo sermon healed into auto-created "Section 1", dangling `series_id` goes standalone, v30–32 columns arrive); **v32 → 33** (Equip → `Manuscript/Body`, Frame → the doors, `last_touched_position` rewrites, `last_manuscript_subphase` seeded, `last_assembly_subphase` falls back to Outline); malformed structured JSON (fails SOFT — raw text indexed, ladder completes); v13 with silently-missing columns (v14 reconciliation heals); **mid-ladder throw** (sabotaged `sermon_search` schema → the WHOLE transaction rolls back: version, DDL, and rows untouched; the file reopens `quick_check`-clean and completes once unblocked); corrupt primary + valid `.bak` (restored, damaged generation quarantined, stale WAL can never replay); missing primary + orphaned sidecars (cleared before restore); legacy candidate selection (3-row old library beats 1-row newer one AND a 5-row sample-only one; mtime breaks ties only); idempotent re-run (second `migrate()` returns false, changes nothing).
- **`electron/dbRecovery.cjs`** — initDatabase's Phase-1 recovery (corruption classification, quick_check probe, `.bak` restore, quarantine + sidecar hygiene, content-row counting) moved verbatim behind `createDbRecovery({ BetterSqlite3, fsImpl, logger, supportEmail, sleep })`; main.js composes it in the same order as before. Orchestration in main.js itself stays source-scanned (honest gap, unchanged).
- **Recovery-point objective ruled and aligned:** RPO = one app launch (`.bak` is a boot-time copy). The pastor-facing recovery warnings now say "anything you added or changed since the last time SermonForge started may be missing" — the old "one or two edits may be missing" wording promised a recovery point the architecture never had. Cadence deliberately unchanged (WAL already makes committed writes crash-durable). Ruling + rationale in `docs/SYSTEMS/database.md` ("Backup + recovery-point objective"); tests pin the honest wording.
- Known drift corrected: `docs/SYSTEMS/database.md` schema version 32 → **33**. No schema change, no synchronous-mode change, no backup-cadence change. Suite 464 → 474 green; lint 0; spine-integrity OK (125 files); `npm start` boots clean on the extracted recovery.

---

## 2026-07-13 — fix: make sermon and planner mutations atomic (remediation session 3/6)

- **Part A — source + search commit or roll back together.** Every mutation that changes searchable sermon state now runs its source rows AND its `sermon_search` projection in ONE SQLite transaction (`withTransaction`, `electron/persistence.cjs`): create-sermon (whose post-commit index failure used to strand a committed row while IPC reported failure — the renderer retry then created a duplicate), update-sermon, apply-mutation, soft delete, restore, series-title change (per-sermon re-index rides the same txn — no half-renamed search rows), series deletion, section deletion, sample reset. `rebuildSearchIndex()` added as the explicit whole-projection REPAIR mechanism; never the normal write path.
- **Part B — one visible planner gesture, one transaction.** Three bounded spine ops replace renderer `Promise.all` fan-outs: `reorder-sections`, `reorder-series-sermons` (topical arrangement), and `bulk-date-sermons` (all dates + the `series.end_date` mirror in the same txn; resolves `{ end_date }` so the planner's optimistic series state settles on DB truth). Single date picks and Suggest Sundays both ride `bulk-date-sermons`; a failure reloads unchanged database truth (`reloadPlannerTruth`). Renderer optimistic behavior preserved; no visible redesign.
- **Part C — main-boundary relational validation.** Parents must exist (`series_id` on create-sermon/create-section; targets of update/delete/restore), series/section combinations must cohere (`STATE_1_SECTION_SERIES_MISMATCH`, `STATE_1_INCOHERENT_PARENT`), zero-row updates reject (`NOT_FOUND`) instead of silently no-oping, and **an unknown supplied field rejects the ENTIRE mutation identically in dev and production** (`STATE_5_UNKNOWN_FIELD`) — the packaged warn-drop-save-siblings-report-success path is gone. Fixture mirrors updated in lockstep; the stale test pinning "known fields still apply" rewritten to pin the new law.
- **14 failure-injection proofs** (`tests/persistence/atomic-mutations.test.ts`) on the production seam against real SQLite, injected with real `RAISE(ABORT)` triggers inside the transactions: create leaves no row / retry yields exactly one; update, soft-delete, restore leave source+search unchanged; series-title mid-loop failure reverts ALL rows; section + topical reorders roll back every sort_order; Suggest-Sundays failure rolls back dates AND the end_date mirror; ghost parents, cross-series sections, ghost update targets, and mixed valid+unknown payloads all reject with nothing saved; healthy paths land unchanged.
- Suite 450 → 464 green; lint 0; spine-integrity OK; `npm start` boots clean; planner Schedule/Outline render in preview with zero console errors. No schema change, no search-behavior change, no command framework.
