...
> **⛔ HISTORICAL RECORD — authority transferred to CORE + WORKSPACE-CANON (Workspace Re-Foundation, 2026-06-15).** ⚠ The trail metaphor this charter built was **deleted wholesale** in the invisible-system rebuild (closed 2026-05-18) — the live surface is the writing surface + map + threshold overlays, NOT a trail. The live authority for the sermon walk is now [`docs/CORE.md`](../CORE.md) (the law) and [`docs/WORKSPACE-CANON.md`](../WORKSPACE-CANON.md) (the walk's what & why); the mechanics are in [`docs/SYSTEMS/sermon-workspace.md`](../SYSTEMS/sermon-workspace.md). This charter is a frozen development record describing a system that no longer exists; it is **not a working guide and does not bind.** Do not rebuild any "trail" / "rail" / "switchback" surface from here.
