...
- **Next:** A2.2 — paraphrase blocks component (read-only main-sentence blocks from canvas + paraphrase fields beneath, paste-blocked).

A1's substrate (storage shape, spotlight UX, N/A escape valve) is now in place across all four sub-phases. A2.0 + A2.1 land the data-layer foundation and the most distinctive new UI pattern (the canvas) that B1's Field 4 wires together. The remaining A1 sub-item (A1.2) is a UX detail that materially depends on B1's multi-question field shapes; tracking it under A1 keeps the scoping intent intact even though its execution slot moves to "after B1 lands."
