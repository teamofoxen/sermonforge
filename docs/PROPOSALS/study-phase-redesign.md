...
- **Next:** B3.2 — Field 5 (Christ-Connection Statement) wire-up. The cumulative-synthesis-table extends a third writable column (`christ_connection`) to `observations.divisions.thought_units` rows. Q1 (`christ_per_unit`) reuses the B2.2 cross-phase plumbing (`crossPhaseRead` / `crossPhaseWrite`) and the same `cumulative-synthesis-table` kind dispatch. Q2 (`statement`) is a text-prompt stored in `redemptive_thread.christ_connection_statement.statement`. Composite gate at the Redemptive Thread → Implications boundary: every thought-unit row has a non-empty `christ_connection` column AND `statement` non-empty. The legacy `summary` field's Synthesize block is removed from StudyTab; its data migrates into `christ_connection_statement.legacy_notes` defensively (no production sermons require migration). The pre-existing B1.0-era bug in StudyTab's Phase 1 / Phase 2 / Phase 3 Review buttons (the inline `getPrimaryAnswer`-based `filled` builder silently skips multi-question content) is a candidate to fix as a small follow-on cut after B3.2 — switch the Review buttons to `flattenToText(data, FIELDS)` so they pick up multi-question content like B1.7 did for context tiers.
