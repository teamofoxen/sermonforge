...
- **Next:** B4.2 — Field 4 (Implications Synthesis) wire-up. Replace the single primary question with the SFDI 2-question sequence: Q1 `implication_per_unit` is a `cumulative-synthesis-table` reusing the B2.2 / B3.2 cross-phase plumbing — extends `observations.divisions.thought_units` with the writable `implication` column on top of Phase 1's three columns + Phase 2's `meaning` + Phase 3's `christ_connection` (all read-only). Q2 `synthesis` is a text-prompt stored in `implications.implications_synthesis.synthesis`. Composite gate at the Implications → MPT/MPS boundary (`evaluateAdvance` with `kind=sub_phase, fromIndex=4` — currently a step-transition reading whole-Exegesis evidence; will tighten to read Field 4's composite). PC card removal from `SermonWorkspace.jsx` ships alongside (per the binding scope decisions; Field 3 carries the substance now). After B4.2 closes: SPRD's structural backlog items can fan out (C1, C2, C4, C5, C6 per § Implementation milestones; C3 waits on SADI).
