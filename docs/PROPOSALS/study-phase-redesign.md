...
- **Next:** B1.6 — A1.2 hover-checklist on the disabled Continue gate. Now that Field 4's composite is live, the gate has a real multi-condition shape (Q1 + Q2 + Q3 + Field 8 + Field 9) — the checklist surfaces each unmet condition rather than only the first failing reason. Touches `evaluateAdvance` (return structured per-gate state alongside the composite reason) and the disabled-Continue rendering at the three sub-phase / step boundaries in `StudyTab.jsx`.
