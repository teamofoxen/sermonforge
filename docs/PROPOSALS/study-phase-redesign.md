...
- **Next:** B2.2 — Field 7 Interpretation Synthesis wire-up. Heavy-lifting field opens with `FieldOverviewScreen` on first per-sermon entry. Q1 (`meaning_per_unit`) is a structured-exercise question that extends Field 4 Q3's synthesis-table sub-shape with a writable Meaning column and Phase 1 columns rendered read-only — reuses A2.3 SynthesisTable's existing `columns` prop with cumulative-column support. Q2 (`meaning_whole`) is a text-prompt question. Composite gate at the Interpret → Redemptive Thread boundary: every thought unit from Phase 1 has a Meaning entry filled, and Q2's whole-passage paragraph is non-empty. The `gates` field at that boundary surfaces a single Field 7 entry with sub-reasons; Phase 2's load-bearing-field count is one (Field 7), so the checklist is single-line at this boundary unless later baseline-tightening adds more. The cumulative thought-unit data layer needs a small architectural decision (Phase 1 owns the spine; Phase 2 either snapshots or reads-and-zips) — to be settled at the start of B2.2.
