...
- **Next:** B1.4 — `evaluateAdvance` per-boundary threshold for Observe → Interpret. SFDI defines the threshold as Field 4 composite + Field 8 non-empty + Field 9 both questions filled. Field 4's composite gate depends on B1.5 wiring (canvas + paraphrase + table data shapes), so a phased landing: B1.4 ships Field 8 + Field 9 gates against the existing data; the Field 4 composite extends `evaluateAdvance` once B1.5 lands. After B1.4: B1.5 (Field 4 wire-up against A2.x primitives — the heaviest cut), B1.6 (Field 6 Commands and Declarations table), B1.7 (composite-gate hover-checklist, closing A1.2).
