...
- **Next:** A2.1 — indented sentence canvas component (Tab/Shift+Tab indent semantics, line-number gutter, level-0 visual marker), landing against A2.0's stable storage shape.

A1's substrate (storage shape, spotlight UX, N/A escape valve) is now in place across all four sub-phases. A2.0 lands the data-layer foundation that A2.1+ canvas/paraphrase/table components and B1's Field 4 three-question composite gate land against. The remaining A1 sub-item (A1.2) is a UX detail that materially depends on B1's multi-question field shapes; tracking it under A1 keeps the scoping intent intact even though its execution slot moves to "after B1 lands."
