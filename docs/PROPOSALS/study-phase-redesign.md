...
A1's substrate (storage shape, spotlight UX, N/A escape valve) plus A2's structured-exercise UI primitives are now in place across all four sub-phases. **Next critical-path move: B1 — Phase 1 (Observe) reshape**, which wires the canvas / paraphrase-blocks / synthesis-table / reference-panel / overview-screen primitives into Field 4 (Divisions / Thought Units), brings up the new fields (Background, Surface Questions), and consolidates Notable Commands + Notable Statements into Commands and Declarations.
