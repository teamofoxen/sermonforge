...
- **Next:** B1.3 — heavy-lifting flag + pre-field overview wiring. Mark Field 1 Background, Field 4 Divisions / Thought Units, and Field 9 Possible Implications as heavy-lifting per SFDI; render `FieldOverviewScreen` (A2.5) on first entry to each. Per-sermon "first entry" tracking via localStorage (mirrors A1.1's `sermonforge_study_subphase_*` precedent).
