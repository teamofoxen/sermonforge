...
- **Next:** B1.1 — multi-question field rendering. Extend the field-def shape to support a `questions: [...]` array per field; extend `SpotlightField` / `SpotlightWorksheet` in StudyTab to render multi-question fields with the spotlight pattern (one question active at a time, prior answers persist visibly, "Next question →" disabled-when-empty). Foundation for B1.2+ wiring of each Phase 1 field's question sequence.
