...
- **Next:** later B1 cuts as needed — Field 6 Commands and Declarations table (single-question with a structured table per SFDI), and per-field non-empty checks on Fields 1/2/3/5/6/7 if B2 walks surface load-bearing tightening as worth doing before the Phase 2 reshape begins.
