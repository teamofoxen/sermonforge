...
- **Next:** B1.5 — Field 4 (Divisions / Thought Units) wire-up. The heaviest B1 cut. Mount the A2.x primitives — `IndentedSentenceCanvas` for Q1, `ParaphraseBlocks` for Q2, `SynthesisTable` for Q3 — inside Field 4's spotlight, with the `PeripheralReferencePanel` beside Q1 carrying the SFDI three rules + genre-specific outline tips ("For epistles" / "For narrative"). Wire the composite gate into `evaluateAdvance`: Q1 satisfied when canvas has at least one main sentence with at least one indented modifier; Q2 satisfied when every paraphrase block has content (or the canvas is empty); Q3 satisfied when the table has at least one complete row (Thought unit + After line filled, Signal optional for the final unit). Update Field 4's `questions` array in `OBSERVE_FIELDS` with the three keys (`sentence_layout`, `paraphrases`, `thought_units`) plus a `kind` discriminator the SpotlightField uses to mount the right primitive instead of a textarea.
