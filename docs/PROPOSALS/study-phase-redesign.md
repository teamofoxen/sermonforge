...
- **Next:** B2.2 — Field 7 Interpretation Synthesis wire-up. Heavy-lifting field opens with `FieldOverviewScreen` on first per-sermon entry. Q1 (`meaning_per_unit`) is a cumulative-column extension of Field 4 Q3's synthesis table — same canonical thought-unit array in `observations.divisions.thought_units` (Phase 1 owns the spine; all four phases read+write this one array, per the option-1 architectural decision). Q2 (`meaning_whole`) is a text-prompt question. Composite gate at the Interpret → Redemptive Thread boundary: every thought unit has a Meaning entry filled, Q2's whole-passage paragraph is non-empty. New question kind `cumulative-synthesis-table` and a small SpotlightWorksheet plumbing extension (`crossPhaseRead` / `crossPhaseWrite` props, plus a `crossPhaseSource` declaration on the question) handle the cross-column read/write. StudyTab plumbs both for Phase 2's worksheet only.
