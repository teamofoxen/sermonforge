...
*Component 1 — extension for structured-exercise questions (added 2026-05-03 from SFDI Field 4 walk).* SFDI's Field Pattern addendum recognizes a second question type alongside the default text-prompt: **structured-exercise questions** — a canvas the pastor works inside rather than a textarea they write into. Field 4 (Divisions / Thought Units) is the first walked example: its Q1 is an indented sentence-layout canvas; its Q2 is a small follow-up table that references Q1's line numbers. Component 1's spec extends to cover the affordances structured-exercise questions require. The eight items below are drawn from the Field 4 mockup and generalize to future structured-exercise fields.
...
5. **Paste-intercept enforcement — per-field opt-in property.** A field can declare "no copy-paste in this canvas." When set, the keyboard paste shortcut is intercepted with a quiet inline message; drag-and-drop of text is rejected. Per-field, not app-global. Field 4 sets this property — its by-hand-typing philosophy depends on the enforcement.

6. **Composite per-field advancement gating.** The per-field empty-evidence override (SFDI Field Pattern) needs implementation richer than "field has any answer." For Field 4: the "Next question" button activates only when (a) the canvas has at least one main sentence (level 0) with at least one indented modifier under it AND (b) the divisions table has at least one non-empty row. Component 1 supports per-field gating expressions of this shape. Hover-surfacing of a per-move checklist (so the pastor knows which criterion is missing) is a candidate enhancement.

7. **Storage shape for structured-editor questions — open decision.** Two options for the indented document Q1 produces:
   - **Single string with whitespace preserved.** Simple; loses sub-addressability for downstream prompts.
   - **Structured array** `[{type, text, depth, parent_index}, ...]`. Richer; sets precedent for structured-field storage; requires a reader for the AI prompts that consume it.

   The decision lands when Component 1's structured-exercise work goes from spec to implementation. SFDI declares the field's existence; SPRD owns the storage choice.
