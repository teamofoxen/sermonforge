...
- **Next:** A2.5 — pre-field overview screen (heavy-lifting fields open with a framing screen — title + subtitle + body + Begin button — shown only on first entry to the field for a given sermon).

A1's substrate (storage shape, spotlight UX, N/A escape valve) is now in place across all four sub-phases. A2.0–A2.4 land the data-layer foundation and the canvas / paraphrase-blocks / synthesis-table / reference-panel UI pieces that Field 4's three structured-exercise questions plus its peripheral guidance need. The remaining A1 sub-item (A1.2) is a UX detail that materially depends on B1's multi-question field shapes; tracking it under A1 keeps the scoping intent intact even though its execution slot moves to "after B1 lands."
