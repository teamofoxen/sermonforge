...
- **Next:** A2.4 — persistent peripheral reference panel (28% horizontal, never-collapse content-only panel beside a structured-exercise canvas surface; genre-aware static reference content per Field 4 / future heavy-lifting fields).

A1's substrate (storage shape, spotlight UX, N/A escape valve) is now in place across all four sub-phases. A2.0–A2.3 land the data-layer foundation and the canvas / paraphrase-blocks / synthesis-table UI pieces that Field 4's three structured-exercise questions need. The remaining A1 sub-item (A1.2) is a UX detail that materially depends on B1's multi-question field shapes; tracking it under A1 keeps the scoping intent intact even though its execution slot moves to "after B1 lands."
