...
- **Next:** A2.3 — multi-column synthesis table component (Field 4 Q3 sub-shape: Thought unit | After line | Signal columns; `after_line` autocomplete from canvas line numbers; per-cell no-AI policy; paste allowed because synthesis is the discipline).

A1's substrate (storage shape, spotlight UX, N/A escape valve) is now in place across all four sub-phases. A2.0 + A2.1 + A2.2 land the data-layer foundation and the canvas / paraphrase-blocks UI pieces that Field 4's first two questions need. The remaining A1 sub-item (A1.2) is a UX detail that materially depends on B1's multi-question field shapes; tracking it under A1 keeps the scoping intent intact even though its execution slot moves to "after B1 lands."
