...
- **Merida as conversation partner, not script — interlocutor for every field.** The pastor remains the source-of-truth. **Every field walk — across every sub-phase, every time — opens with a Merida-anchored question.** Merida is the interlocutor for each field; his framing of the field starts the conversation, through Claude. The question is a thought-starter, not a template the answer must fit. The pastor responds in their own voice — what the field is for, what they write in it, how it sets up the next. Claude captures, sharpens with follow-up questions, surfaces alignment or divergence with Merida as a check, and shapes the six-slot entry (Name, Intent, What gets written, Role in sub-phase, Connects from, Connects to) collaboratively from the pastor's words. Where Merida and the pastor's articulation diverge, the pastor decides. The reference at `memory/project_cce_merida_source.md` and the "Study throughline" section in `docs/SYSTEMS/sermon-workspace.md` carry Merida's content for the per-field opener and ongoing cross-reference. SermonForge's structure is Merida-derived, not Merida-faithful.
