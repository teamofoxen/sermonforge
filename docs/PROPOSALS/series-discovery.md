...
> **Status: BUILT on `feature/series-discovery`, then SIMPLIFIED by the
> 2026-07-22 product-owner ruling below.** The *what & why* of the Discovery
> front screen of the Series Planner, and the smallest data + architecture
> changes it required. This is a working note, not a sprawling charter. The live
> authority remains [`docs/CORE.md`](../CORE.md), and the planner mechanics live
> in [`docs/SYSTEMS/series-planner.md`](../SYSTEMS/series-planner.md);
> where they speak, they govern. Recommended design is chosen, not surveyed.
> The sections below describe the product as ruled — the original eight-step
> brief is preserved in this file's git history and named in the ruling.
...
Outline displays and edits the resulting Book ▸ Major Section ▸ Sermon plan; the
Sermon Workspace prepares each sermon in it. Standalone sermons are untouched.
...
## 0. Product-owner simplification ruling — 2026-07-22

Series Discovery is a **straightforward exegetical worksheet made native to
SermonForge — not a workflow platform.** The initial build (same branch, same
day) shipped an eight-step walk; a completed product/UX audit found it heavier
than the product it serves, and this ruling simplified the visible experience
while preserving the shared-truth architecture whole. What the ruling changed
from the original brief:

- **Difficult Decisions is removed outright** — the step, `DecisionsStep`, its
  three-item cap (`MAX_DIFFICULT_DECISIONS`), `decisionsOf`, and all writes to
  `series.discovery.decisions`. No replacement; uncertainty management is not
  the worksheet's job. Old envelopes may still carry a `decisions` key; the
  fail-soft parse ignores it (no migration).
- **The walk is seven parts:** Immerse in the Book · Understand the Book · Map
  the Major Sections · Identify the Preaching Texts · Review the Preaching
  Texts · Propose the Series Big Idea · Planner-Ready Output.
- **"Major Sections" is the only pastor-facing term for the middle tier** —
  "Major Movements" is retired everywhere live. A major section in Discover is
  not *translated into* an Outline section; it **is** one, so the
  "becomes a section" copy is gone too. Preaching texts read explicitly as the
  smaller literary units *within* a major section.
- **The review step is a simple review, not an audit** — third-person prompts,
  no checkboxes, no completion states, no stored review status, no scores.
- **Discovery carries no coverage surface.** `CoveragePanel` was removed from
  Steps 5 and 8 (now 5 and 7); coverage remains on the Schedule, where the
  shared component still renders.
- **Planner-Ready Output** (renamed from Planner-Ready Review) is a clean read
  of the plan, not a grading report — missing fields read "Not written yet" in
  quiet neutral styling, never crimson.
- **Classroom parentheticals** — "(subject)", "(complement)" — are dropped from
  the preaching-text reasoning labels.

Everything else stands: the shared canonical records and mutation paths, schema
v34 and the per-entity `discovery` envelopes, the no-section guard, the landing
and re-entry rules, the Series Big Idea candidate flow, and the topical /
standalone journeys.
...
step (seven parts, per the 2026-07-22 ruling):

1. **Immerse in the Book** — read it whole, repeatedly, as one complete work;
   look for repeated words and phrases, commands, warnings, promises, examples,
   contrasts, transitional words, and changes in subject, tone, audience, or
   argument. (No sermon divisions yet.) A light notes space; nothing required.
...
3. **Map the Major Sections** — he *explicitly creates* each major section (a
   real Series Section). Per section: Title · Passage · **why it begins here** ·
   **why it ends here** · Big Idea · Overview.
4. **Identify the Preaching Texts** — within each major section he *explicitly
   creates* each preaching text (a real Sermon under that Section): the smaller
   literary units that will become sermons; most major sections hold several.
   Per text: Passage · why it begins/ends here · what the author is *talking
   about* · what he is *saying about it* · what he is *doing* (a fixed list he
   picks from, or Other) · Big Idea · Overview · Working Title.
5. **Review the Preaching Texts** — a simple review against plain questions
   (one coherent movement of thought? beginning/ending make sense? command kept
   with its reason? too large / too small? big idea accounts for the whole
   passage? the text driving the plan?) over a plain readout of the texts. No
   checkboxes, no stored state, no scores, no coverage readout.
6. **Propose the Series Big Idea** — after the map exists: the burden, what keeps
...
7. **Planner-Ready Output** — a clean read of the plan he produced (book · major
   sections · sermons). Anything unwritten reads "Not written yet" in quiet
   neutral styling. Not a gate, and not a grading report; it closes with the
   plan being live in Outline and an explicit Open Outline action.
...
- Inside Discover, the seven steps are a labeled stepper with Back / Next and
  ...
  an out-of-range saved step — e.g. one written by the retired eight-step walk —
  clamps to the nearest live step.
- Discover says, in plain words, that it holds **the questions behind the plan**
  while Outline shows **the clean plan those answers produced** — the same major
  sections and sermons, one plan, two views. (There is no "becomes a section"
  translation copy: a major section here already *is* an Outline section.)
...
- Creating a **major section** calls `spine.createSection` — a real
...
- Creating a **preaching text** calls `spine.createSermon` under an existing
  major section — a real `sermons` row, the same draft→commit path Outline uses,
  and **always with `section_id`** (never section-less: a section-less in-series
  book sermon makes the spine auto-fabricate a "Section 1" the pastor never asked
  for).
- The **canonical fields** — section Title / Passage / Big Idea / Overview; sermon
  ...
  book node edits.
- Deletes and reorders run the existing spine ops, so a section/sermon removed in
...
- **`series.discovery`** — `readNotes` (1); `understandWhyWritten`,
  ...
  `understandWantsReaderTo` (2); `bigIdeaBurden`, `bigIdeaRecurring`,
  `bigIdeaResponse`, `bigIdeaUnifier`, `bigIdeaCandidateA`, `bigIdeaCandidateB` (6).
  (A retired `decisions` array from the removed Difficult Decisions step may
  linger in envelopes written before the 2026-07-22 ruling; nothing reads or
  writes it, and the fail-soft parse ignores it — no migration.)
...
- **`sermons.discovery`** — `{ whyBegin, whyEnd, subject, complement,
  ...
  talking-about/saying-about/function; `authorialFunction` is one of a fixed
  vocabulary the pastor *picks*, or "Other" + the free text — never a suggestion).

Steps 5 and 7 store **nothing** — they are read-only views over the canonical
plan.
...
- Lift the entity-mutation orchestration (`addSection`, `addSermon`/`commitDraft`,
  ...
  props each.
- New file **`src/components/SeriesDiscover.jsx`** — the seven-part walk. It uses
  the shared mutation functions for section/text creation and the parent field
  savers for every write — so Discovery inherits the topbar Saving / Saved /
  Save-failed + Retry, flush-on-exit, and leave-guard for free (Mutation #3). Any
  new debounced saver it introduces registers with `useFlushOnExit`. (It renders
  no coverage — `CoveragePanel` stays a Schedule surface per the 2026-07-22
  ruling.)
...
2. Record whole-book **Immerse** notes and **Understand** answers (Discovery notes).
3. **Explicitly create a major section** → it is a real Section, visible
   immediately in **Outline**; record its boundary reasoning.
4. **Explicitly create a preaching text** within that major section → a real
   Sermon under that Section, visible immediately in Outline; record its boundary
   reasoning, what the author is talking about and saying, and his function.
5. Author every Big Idea and Overview himself; the review questions appear at
   Step 5 as a simple review — nothing stored, checked off, or scored.
...
7. **Planner-Ready Output** gives a clean read of the plan, marking unwritten
   fields "Not written yet" in neutral styling, and closes with Open Outline.
...
- No AI or rule-based pseudo-AI: no suggested divisions, sections, texts, big
