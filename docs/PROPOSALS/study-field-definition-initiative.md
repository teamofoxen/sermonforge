...
> **⛔ HISTORICAL RECORD — authority transferred to CORE + WORKSPACE-CANON (Workspace Re-Foundation, 2026-06-15).** The live authority for the sermon walk is now [`docs/CORE.md`](../CORE.md) (the law) and [`docs/WORKSPACE-CANON.md`](../WORKSPACE-CANON.md) (the walk's what & why — every question, named outcome, completeness policy, Merida tag). This document is the frozen development record of how that shape was reached; it is **not a working guide and no longer binds.** Any "remains binding" / "live" / "production" language below is itself historical (including the per-banner "structural commitments remain binding" lines and the N/A ruling, now seated in CORE Process #2 + WORKSPACE-CANON §5). Where this doc and CORE/canon differ, CORE/canon win — do not rebuild from here.
...
>
> **PASTOR RULING — RESOLVED 2026-06-14 (Re-Foundation Phase 1, examination 1): N/A is RESTORED to Study.** The pastor ruled the honest-absence gesture back in — a preacher may mark a Study question "doesn't apply to this passage" (and a per-thought-unit cell "nothing here") and have it count as done, so the completeness signal never rewards manufacturing a connection the text doesn't carry. This document's N/A grants for the declared Study questions (Observe Where/When; Genre field-level; Cross-References/Commentary; RT's four-ways questions; Implications Fields 1–2) are therefore **RE-INSTATED** — the resolution is *extend*, not amend-down. **One hard exclusion holds (not the pastor's to relax):** the named-outcome paragraphs — the Christ-Connection Statement and both syntheses (`meaning_whole`, CCS `statement`, both `synthesis` keys), plus the MPT/MPS `tighten` questions the Word export reads — stay NO-N/A, because they are the load-bearing artifact and the old everywhere-toggle's hole (an N/A'd named outcome counting as answered with no text behind it) must stay closed. **Scope / timing:** the ruling is settled law and lives in CORE Process #2 + `docs/WORKSPACE-CANON.md` §5. The shipped code still enforces the two-question allowlist (`intro.redemptive_note`, `mps.gospel_check`); restoring the per-question and per-cell N/A gesture in the app is a scoped code change tracked separately (not yet built). Until then this records the decided target, not the current code.
