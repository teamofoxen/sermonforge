...
- **Intent:** Integrate the three-way conversation into the named outcome. Per-thought-unit integrated implication extends the cumulative thought-unit table with a sixth column. The synthesis paragraph itself is the named outcome — one paragraph in the pastor's own voice integrating Theological Significance + Personal Implications + Pastoral Context. The marinate-output Merida calls for. The substrate MPT/MPS opens against.
...
**Marinate moment restored:** the Implications Synthesis IS the marinate-output. Merida explicitly tells the pastor to back away after Implications and ponder before crafting the sermon. SermonForge previously jumped to MPT/MPS with an AI four-phase synthesis; now the four named outcomes (the pastor's own articulations, in their own voices, accumulated across the four sub-phases) ARE the marinate substance, and MPT/MPS opens against them directly.
...
**Marinate moment:** the Implications Synthesis IS the marinate-output. The pastor finishes Phase 4 with a paragraph synthesizing the three voices — and that paragraph (plus the cumulative table, plus the prior three named outcomes) is what they sit with before crafting the sermon. SPRD's structural commitment around how the marinate-pause manifests in the workspace UX (a "Save and step away" pattern, a deliberate slow-down at the boundary, etc.) is downstream work; SFDI's content commitment is that the synthesis IS the substance to marinate on.
