...
| `discovery` | TEXT (JSON) | Series-level Discovery-only reasoning envelope, authored in the Discover walk (book series only). Flat keys: `readNotes` (Step 1); `understandWhyWritten` / `understandSituation` / `understandProblem` / `understandResponse` / `understandWantsReaderTo` (Step 2); `decisions` (Step 6, ≤3, each `{id,optionA,optionB,evidenceA,evidenceB,preference,why}`); `bigIdeaBurden` / `bigIdeaRecurring` / `bigIdeaResponse` / `bigIdeaUnifier` + the two candidates `bigIdeaCandidateA` / `bigIdeaCandidateB` (Step 7). Nullable; fail-soft parse (`src/utils/discovery.js`). The FINAL canonical Series Big Idea stays `big_idea` and the Overview stays `overview` — this holds only the working-out. Persisted via `updateSeries`, never the create INSERT. (v34) |
