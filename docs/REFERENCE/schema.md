...
| v34 | Series Discovery — one nullable JSON `discovery` column on **each** of `series`, `series_sections`, and `sermons`, holding the Discovery-only *reasoning* the pastor authors in the Discover walk that cannot live in a clean planner field (major sections are real sections, preaching texts are real sermons — their canonical Title/Passage/Big idea/Overview are shared, not duplicated). Each envelope is a **flat** object of well-known keys (fail-soft parse via `src/utils/discovery.js`, so a single-field merge can't drop a sibling): `series.discovery` = `{ readNotes, understandWhyWritten, understandSituation, understandProblem, understandResponse, understandWantsReaderTo, bigIdeaBurden, bigIdeaRecurring, bigIdeaResponse, bigIdeaUnifier, bigIdeaCandidateA, bigIdeaCandidateB }` (a `decisions` array shipped with the original eight-step walk and was retired by the 2026-07-22 simplification — old envelopes may still hold it; the fail-soft parse ignores it, no migration); `series_sections.discovery` = `{ whyBegin, whyEnd }`; `sermons.discovery` = `{ whyBegin, whyEnd, subject, complement, authorialFunction, authorialFunctionOther }`. All three additive + nullable (no backfill, no columns dropped); added to `SERIES_COLUMNS` / `SECTION_COLUMNS` / `SERMON_COLUMNS` (3 mirrors each), never the create INSERT (rides create-then-update via `updateSeries/Section/Sermon`). `assertSchemaContract` extended to also check `SECTION_COLUMNS` (its first writable column beyond the CREATE TABLE set). `discovery` is NOT in `sermon_search` (reasoning, not manuscript content). |
...
| `discovery` | TEXT (JSON) | Series-level Discovery-only reasoning envelope, authored in the Discover walk (book series only). Flat keys: `readNotes` (Step 1, Immerse); `understandWhyWritten` / `understandSituation` / `understandProblem` / `understandResponse` / `understandWantsReaderTo` (Step 2); `bigIdeaBurden` / `bigIdeaRecurring` / `bigIdeaResponse` / `bigIdeaUnifier` + the two candidates `bigIdeaCandidateA` / `bigIdeaCandidateB` (Step 6). A retired `decisions` array (the removed Difficult Decisions step, 2026-07-22 simplification) may linger in old envelopes; nothing reads or writes it and the fail-soft parse ignores it. Nullable; fail-soft parse (`src/utils/discovery.js`). The FINAL canonical Series Big Idea stays `big_idea` and the Overview stays `overview` — this holds only the working-out. Persisted via `updateSeries`, never the create INSERT. (v34) |
...
| `sort_order` | INTEGER | |
| `created_at` | TEXT | |
| `discovery` | TEXT (JSON) | Major-section Discovery-only boundary reasoning: `{ whyBegin, whyEnd }` — *why the section begins here* / *why it ends here*, authored in the Discover walk's Map-the-Major-Sections step. Nullable; fail-soft parse. A major section **is** this section row; its canonical Title / Passage / Big idea / Overview are the columns above (shared with Outline), so only the boundary reasoning is new. Persisted via `updateSection`, never the create INSERT. Section allowlist's first writable column beyond the CREATE TABLE set (so `assertSchemaContract` gained a `series_sections` check in v34). (v34) |
...
| `discovery` | TEXT (JSON) | Preaching-text Discovery-only reasoning envelope (flat keys): `{ whyBegin, whyEnd, subject, complement, authorialFunction, authorialFunctionOther }` — *why the passage begins/ends here*, what the author is *talking about* (subject), what he *says about it* (complement), and what he is *doing* (`authorialFunction` ∈ the fixed `AUTHORIAL_FUNCTIONS` vocabulary in `src/utils/discovery.js`, or `"Other"` with the free text in `authorialFunctionOther`). Authored in the Discover walk's Identify-the-Preaching-Texts step. Nullable; fail-soft parse. A preaching text **is** this sermon; its canonical Passage / Working Title / Big idea / Overview / `section_id` are the columns above (shared with Outline), so only the exegetical reasoning is new. Persisted via `updateSermon`, never the create INSERT (a draft holds it until `commitDraft` carries it forward). NOT in `sermon_search` (reasoning, not manuscript content). (v34) |
