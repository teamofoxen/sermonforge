...
- **`scripts/theology/ingest/run.py`** — one-command wrapper: `python scripts\theology\ingest\run.py corpus\manifest\<work>.yaml`. Orchestrates in order: fetch (skip if raw_path exists and sha256 matches) → parse (skip if paragraphs.jsonl newer than raw) → chunk (skip if chunks newer than paragraphs) → load (already idempotent: delete-then-insert by work_id) → FTS rebuild → vectors (incremental). Flags: `--force` re-runs all stages regardless of freshness; `--skip-vectors` for faster dev loops. Bails with a clear error if any stage fails. If you drop the XML yourself, the fetch step is a no-op — the manifest still records `source_url` for provenance but the script trusts the local file.
- **`scripts/theology/ingest/scaffold_manifest.py`** — inspects a ThML XML file and emits a draft manifest (author/work/translator from `<generalInfo>` + `<printSourceInfo>`, structure block inferred from the div tree, sha256 computed, Roman vs arabic guessed from `@n` samples, multi-work files flagged). What it cannot reliably guess: which work to scope to when multiple share one file; locator convention edge cases; whether to strip chapter-title `<p>`s; tier assignment. In practice: drop XML → `scaffold_manifest.py` → eyeball/edit 2–3 fields → `run.py`. Two commands, ~30 seconds of YAML review. Once written, the manifest is the only human step for any future work.
