...
**Last verified:** 2026-04-30 (post-triage vocabulary completion)
**Verified against:** `docs/CORE.md`, enforcement pass closed against working tree at HEAD `23a97f3` (uncommitted, see closing summary). Pilots C/D/E/B.2/B.3 closed Surface Contract + State #6. Post-triage hardening: `react/jsx-no-undef` and `no-undef` enabled (closes import-drift class). Vocabulary completion: view keys migrated to `VIEW.{Dashboard,Sermons,Calendar,CompletedSermons,Planning,SeriesPlanner,Workspace}` (PascalCase, single source of truth in `src/core/contracts.ts`); workspace tab keys migrated to `STAGE.{Study,Blueprint,Manuscript,Delivery}` with coordinated update of `contextBuilder.js` switch cases, `reviewPrompts.js`, `prompts/sermon.js`, `memory.js` capture set, and `workspaceTourStops.js`. The `canonical-stage-name` lint rule's forbidden set expanded to include `planning`, `active`, `study`, `outline` (in addition to `writing`, `ready`, `archived`); rule also gained a CSS-class-context exemption so `className={... ? "active" : ""}` literals don't false-fire.
...
**Approach A** (lint-staged in `.husky/pre-commit`) means the residual baseline doesn't block commits on untouched files. New violations on staged files do block. Post-vocabulary-completion the visible total is still **15** (residual `no-raw-button` only — nav/tab/tertiary text buttons), down from the original 185. The expanded `canonical-stage-name` forbidden set surfaces zero violations because the corresponding migrations (view keys, workspace tab keys, AI-loading state keys, DeliveryTab panel keys, step-pill status values) all landed in the same commit.
