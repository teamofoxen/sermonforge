#!/usr/bin/env bash
# Mechanical validator for .claude/skills/end-session/SKILL.md
# Detection-only. Exits 0 iff all mechanical checks pass.

set -u

TARGET="C:/Projects/SermonForge/.claude/skills/end-session/SKILL.md"
EXPECTED_NAME="end-session"
EXPECTED_DIR="end-session"
...
echo "=== end-session SKILL.md mechanical validator ==="
echo "Target: $TARGET"
echo
...
# Skill itself does NOT define a banned-phrases list to enforce on output.
# CLAUDE.md sets CHANGELOG rules referenced by STEP 3 (max 5 bullets, etc.)
# but those are output rules for the agent, not phrases banned in SKILL.md.
