...
export default function StudyTab({ sermon, onUpdate, onAI, aiLoading, onStepChange, onTabChange }) {
  ...
  async function generateMPS() {
    ...
    try {
      ...
      const resp = await sendAIMessage(
        [{ role: "user", content: `Passage: ${sermon.passage || "unknown"}\n\nMPT: ${sermon.mpt}\n\nRedemptive Thread:\n${redThread || "(none)"}\n\nImplications:\n${implications || "(none)"}${piBlock}\n\nDraft a Main Point of the Sermon (MPS). The MPS is a single present-tense sentence. The MPT is the theological anchor — not a template to restate. Do not mirror the MPT's language; ask what it makes possible for these people right now.${piBlock ? " The sermon intro will move the congregation from the cultural world inward through the room to the claim — assume that journey has been made. The MPS does not retrace it. The MPS lands at The Sermon's Work: telegraph that claim, aimed at who is in the room. The Cultural Moment and The Room inform tone and angle only — they do not need to appear in the sentence. When drawing on the pastoral context, name the underlying condition the details reveal, not the specific examples. If the MPT has sequential movements, build them as a causal chain — one subject, one verb, subordinate clauses that carry the reader forward." : " If the MPT has sequential movements, build them as a causal chain — one subject, one verb, subordinate clauses that carry the reader forward."} Aim for 35–45 words. Compress ruthlessly. Return only the sentence.` }],
        "You are a homiletics consultant helping a pastor crystallize a sermon claim. The MPT is the kernel. The MPS is what it produces aimed at this congregation. The sermon intro will handle the concentric journey — cultural world, then the room, then the threshold. The MPS lands after that journey: it is the claim waiting at the end. Derive it primarily from The Sermon's Work; let The Cultural Moment and The Room shape tone and angle without appearing in the sentence. Do not restate the MPT. Do not retrace the intro. One sentence, one spine, landing at the claim."
      );
