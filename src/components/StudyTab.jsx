...
export default function StudyTab({ sermon, onUpdate, onAI, aiLoading, onStepChange, onTabChange }) {
  ...
  async function generateMPS() {
    ...
    try {
      ...
      const resp = await sendAIMessage(
        [{ role: "user", content: `Passage: ${sermon.passage || "unknown"}\n\nMPT: ${sermon.mpt}\n\nRedemptive Thread:\n${redThread || "(none)"}\n\nImplications:\n${implications || "(none)"}${piBlock}\n\nDraft a Main Point of the Sermon (MPS). The MPS is a single present-tense sentence. The MPT is the theological anchor — not a template to restate. The MPS should not translate the MPT into present tense; it should ask what the MPT's claim makes possible for these people right now, and speak that. Do not mirror the MPT's language — let it generate something new aimed at the congregation.${piBlock ? " Move through three concentric circles: (1) enter the cultural world in the background — start where the congregation lives; (2) narrow to this specific room and what they carry in; (3) land the text's claim on that ground. Earn the theology — don't open with it. If the MPT has sequential movements, build them as a causal chain where each clause is the consequence of the prior, not a parallel list. Aim for a single grammatical spine — one subject, one verb — with subordinate clauses that carry the reader forward." : " If the MPT has sequential movements, build them as a causal chain rather than a parallel list. Aim for a single grammatical spine — one subject, one verb — with subordinate clauses that carry the reader forward."} Aim for 35–45 words. Compress ruthlessly — if a clause can be cut without losing a movement in the chain, cut it. Return only the sentence.` }],
        "You are a homiletics consultant helping a pastor move from text to congregation. The MPT is the kernel — it carries the theology. The MPS is what that kernel produces when planted in this soil: this cultural moment, this room, this pastoral purpose. Do not restate the MPT in present tense. Ask what the text's claim makes possible for these people right now, and say that. When pastoral context is present, enter the congregation's world first and earn the theological landing. Express sequential logic as a chain, not inventory. One grammatical spine; everything else subordinate and in motion. When drawing on the pastoral context, identify the underlying condition the details reveal — do not echo the PI's specific vocabulary. Name what the examples point to, not the examples themselves."
      );
  ...
  async function suggestOutline() {
    ...
    try {
      ...
      const resp = await sendAIMessage(
        [{ role: "user", content: `${exegesisContext}\n\nPropose a sermon outline. Return only a numbered list of 3–4 points. Each point must be a single declarative sentence that derives from the text's own logic and ladders up to the MPS. If the MPS is built as a causal chain — where each clause leads to the next — treat those movements as the natural scaffold for the outline points. Each point should carry one movement of that chain forward, so that the outline is the MPS unpacked, not a separate structure imposed alongside it.` }],
        "You are a homiletics consultant helping a pastor structure a sermon. The outline must emerge from the text's argument — not be imposed on it. Each point is a standalone claim the congregation can grasp and remember. When the MPS has a clear causal or sequential structure, the outline points should map to its movements — the outline is the MPS opened up, each point developing one step in the chain."
      );
      if (!resp?.trim()) return;
